...
def request_registration_otp(payload: RegistrationOtpRequest, db: Session):
    ...
    for otp_entry in active_otps:
        ...
        otp_entry.updated_at = now_ist_naive()
        db.add(otp_entry)
    ...
    otp_entry = RegistrationOtp(
        ...
        otp_code=otp_code,
        expires_at=now_ist_naive() + timedelta(minutes=settings.registration_otp_expire_minutes),
    )
    db.add(otp_entry)
    try:
...
def verify_registration_otp(payload: RegistrationOtpVerifyRequest, db: Session):
    ...
    if not otp_entry or otp_entry.expires_at < now_ist_naive():
...
def revoke_session(user: User, session_id: str, db: Session) -> None:
    ...
    session.updated_at = now_ist_naive()
    db.add(session)
    db.commit()
